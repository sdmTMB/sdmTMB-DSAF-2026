...
-   Learn how to look at, predict, and plot output from the two linear predictors
...
Delta (or hurdle) models have one model for zero vs. non-zero data and another component for the positive component. Some of these delta models can be implemented by fitting the two components separately and combining the predictions or they can be fit at once in sdmTMB.
...
-   some (e.g., Poisson-link delta models) must be fit at once because the linear predictors are linked
...
1.  Do these residuals represent the full model or just one linear predictor?
...
1.  What did we just do wrong here? Did we get a warning message? How does our QQ plot look?
