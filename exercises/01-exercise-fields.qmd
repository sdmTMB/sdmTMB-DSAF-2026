...
The spatial range is the distance at which two data points are effectively independent. Technically, this is the distance at which correlation declines to approximately 0.13. Sometimes that's rounded to 10%.
...
The basic function for simulating random fields from scratch in sdmTMB is `sdmTMB_simulate()`.
...
-   The two important standard deviation parameters are
    ...
    -   `sigma_O`: controlling the marginal standard deviation of `omega_s`, the spatial random field values

    -   `phi`: controlling the observation error standard deviation.
...
sim_dat <- sdmTMB_simulate(
  ...
  sigma_O = 0.5, #< try changing this (spatial standard deviation)
  phi = 0.1, #< try changing this (observation error standard deviation)
  B = 0, # intercept
  # seed = 123, # try setting the seed to compare changes while keeping the same underlying spatial random field
)
# head(sim_dat)
...
It may help you to set the seed to compare changes while keeping the same underlying spatial random field.
:::

::: {.callout-note appearance="simple"}
Note that the colour scale (magnitude of observed values) may be changing between runs!
:::
...
::: {.callout-note appearance="simple"}
**How are observed values generated?**

```
Linear predictor  →  Link function  →  Expected value  →  Add obs. error  →  Observed
      eta                 g()                mu              +ε                 y
```

In this example the `link = "identity"` therefore: eta = mu
:::
...
1.  How does this simulation differ from the `sim_dat` simulation we did earlier? What is the one addition from the first spatial simulation?
...
What does marginal AIC tell us about these models?

```{r}
AIC(fit_glm2, fit_sdmTMB)
```
