...
2.  How could you fix this with the new `collapse_spatial_variance` in `sdmTMBcontrol()`?. Try that below with `fit_ye2`.
...
```{r}
fit_ye2 <- update(fit_ye, control = sdmTMBcontrol(collapse_spatial_variance = TRUE)) # answer
...
```{r}
fit_ye3 <- update(fit_ye, family = poisson(link = "log")) # answer
...
```{r}
ggeffects::ggeffect(fit_pcod, terms = "depth [0:600 by=2]") |> plot() # answer
