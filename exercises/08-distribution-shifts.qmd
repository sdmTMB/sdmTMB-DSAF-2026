...
This metric summarizes how the distribution shifts with respect to a habitat covariate. Here, we'll use depth. We flip the sign in the plot so deeper values appear lower on the y-axis.
...
To get the overall spatial trends, we need to combine the fixed and random effects from both linear predictors of the delta model. Because we used the Poisson-link, both are in log space and we can simply add them. We *could not* do this with a standard delta model that has a logit and log link. We need to use the Poisson-link delta *or* a model with one linear predictor (e.g., Tweedie, NB2).
