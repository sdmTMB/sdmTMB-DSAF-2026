...
-   Gain experience with anisotropy.
...
4.  The number of folds, and how folds are selected can affect results (default is random selection)
5.  We can manually set the folds if we'd like. Here we will divide the data by year (i.e., folds contain only data from a single year).
...
cat("LPD difference (anisotropic - isotropic):", round(lpd_diff, 2), "±", round(se_diff, 2), "\n")
```

### Exercise:

1.  Which model does LPD suggest has better out-of-sample predictive performance?
2.  How consistent is that result across folds? Does one model perform better in every fold, or are results mixed?
3.  We assigned folds based on year. What happens if you don't do this (i.e., `fold_ids = NULL`)? Why might you want to set the random seed before each cross-validation run when comparing multiple models with random fold assignment?
4.  How else could you use the folds? (Think about spatial structure or forecasting applications)
