...
fit_sm <- sdmTMB(
  density ~ s(year, k = 5) + depth_scaled + depth_scaled2, #<<
  time = "year",
  extra_time = extra_years, #<<
  spatiotemporal = "AR1", #<<
  data = pcod,
...
```{r pred-all}
newdf <- data.frame(
  year = fit_ar1$time_lu$time_from_data,
  X = mean(pcod$X),
  Y = mean(pcod$Y),
...
```{r pred-ar1-plot-eps}
one_yr <- qcs_grid
grid <- purrr::map_dfr(fit_ar1$time_lu$time_from_data, function(i) {
  one_yr$year <- i
  one_yr
})
...
* Smoothers should be used with caution when forecasting  
  .xsmall[(they continue whatever the basis functions were doing, which can be silly)]
