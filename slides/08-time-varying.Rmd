...
.xsmall[
Note: a `0` or `-1` in formula for suppressing global intercept in this case

`time_varying_type = "rw"` (default) estimates the first time step as independent. I.e. **do not place the same covariate in the `formula` argument** (this includes the intercept). This does not apply to other time-varying types in sdmTMB. Sorry!
...
* `time_varying` is a formula for coefficients that follow a random walk or AR1 process over time

--

* Make sure a coefficient isn't in both `formula` and `time_varying` for `time_varying_type = "rw"`, this includes the intercept
