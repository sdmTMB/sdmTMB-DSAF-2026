...
.xsmall[
A need for reproducible methods and simple code for assessments [(Anderson et al. 2019)](https://github.com/pbs-assess/gfsynopsis)
]

.center[
<img src="images/synopsis-lingcod.png" width="56%" />
]
...
.center[
<img src="images/intro_slide2.jpg" width="85%" />
]

And many more! Including Julia Indivero, Joe Watson, Cole Monnahan, Joseph Barss, with code adapted from VAST, glmmTMB, mgcv, and brms

---

# sdmTMB highlights

User-friendly R package for modeling spatial and spatiotemporal processes
...
Widely applicable to species distribution modelling, stock assessment inputs, bycatch models, estimating distribution shifts
...
4. Inspect the model: .blue[`print()`], .blue[`sanity()`], .blue[`tidy()`], .blue[`residuals()`, .blue[`coef()`], ...]

5. Predict from the model: .blue[`predict()`], .blue[`simulate()`]

6. Get derived quantities: .blue[`get_index()`, `get_cog()`, `get_eao()`, `get_range_edge()`, `get_weighted_average()`, ...]
...
Or use the sf package yourself
...
1. Cutoff: minimum allowed distance between vertices (e.g., `cutoff = 10`)

2. K-means algorithm: used to cluster points (e.g., `n_knots = 100`); sensitive to random `seed` argument!

Alternatively, build any fmesher mesh and supply it to the `mesh` argument in `make_mesh()`
...
.tiny[
Ward, E.J. and Anderson, S.C. 2025. Approximating spatial processes with too many knots degrades the quality of probabilistic predictions. bioRxiv. <https://doi.org/10.1101/2025.11.14.688354>.
]
---
...
# add random intercept and slope by group g:
formula = y ~ x1 + s(x2) + (1 + x1 | g)
```
]
...
We will cover less-common but useful families later
